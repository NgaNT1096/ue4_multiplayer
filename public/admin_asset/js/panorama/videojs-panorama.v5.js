...
var BaseCanvas = function BaseCanvas(baseComponent, THREE) {
    ...
    return {
        constructor: function init(player, options) {
            ...
            console.log(localStorage.getItem("timecurrent"));
            if(localStorage.getItem("timecurrent") !== null && localStorage.getItem("poitLat") !== null && localStorage.getItem("poitLon") !== null){
                this.player().currentTime(localStorage.getItem("timecurrent"));
                this.lon = localStorage.getItem("poitLon");
                this.lat = localStorage.getItem("poitLat");
                
                localStorage.removeItem("poitLat");
                localStorage.removeItem("poitLon");
        ...
        addHotspot: function addHotspot(key,pos,geometry,material){

            
            var hotspot = new THREE.Mesh( geometry, material );
            hotspot.position.set(pos[1],pos[2],pos[3]);
            hotspot.name = key;
            hotspot.zIndex = 2;
        ...
        addMultiHotspot: function addMultiHotspot(){
            if(hotspotArray.length >0){
                var ballTexture = THREE.ImageUtils.loadTexture( 'img/hotspot.png' );
                var ballMaterial = new THREE.MeshBasicMaterial( { 
                    map: ballTexture, 
                    transparent : true,
                    side: THREE.DoubleSide
                } );
                var planeGeometry = new THREE.PlaneGeometry(25,25,1,1);
                for(var i=0; i < hotspotArray.length; i++){
                    this.addHotspot(i,hotspotArray[i],planeGeometry,ballMaterial);
                }
            }else{
                //no input for hotspot
            }
        },
        HotSpotRepaint : function HotSpotRepaint(key,currentTime,pos){
            if(pos){
                  if(currentTime >= pos[0] && currentTime < pos[4]){
                      this.group.children[key].visible = true;
                  }else{
                      this.group.children[key].visible = false;
                  }
            }
        },
        HotSpotManager: function HotSpotManager(){
            var timecurrent = this.player().currentTime();
            for(var i=0; i < hotspotArray.length ; i++){
                this.HotSpotRepaint(i,timecurrent,hotspotArray[i]);
            }
        },
        saveLocalStorage: function saveLocalStorage(){
            if (typeof(Storage) !== "undefined") {
                    localStorage.setItem("timecurrent",this.player().currentTime());
                    localStorage.setItem("poitLat",this.lat);
                    localStorage.setItem("poitLon",this.lon);

            } else {

            }
        },
        updateLogoSprites: function updateLogoSprites(){
            var width = window.innerWidth / 2;
            var height = window.innerHeight / 2;
            this.sprite.position.set( - width + 80,   height - 80, 200  ); // top left
        },
        ...
        handleMouseDown: function handleMouseDown(event) {
            ...
            this.raycaster.setFromCamera(this.mouse, this.camera);
                var intersects = this.raycaster.intersectObjects(this.group.children, !0);
                if (intersects.length > 0 ) {
                   // console.log(intersects[0].point);
                    console.log(intersects[0].object.name);
                    localStorage.setItem("link",intersects[0].object.name);
                    this.player().pause();
                    $(".player_wrapper").fadeOut(3000);
                    
                    $("#infoHotSpot").fadeIn(3000);
                    if(localStorage.getItem("link") !== null){
                          $("#imgInfo").attr("src","img/"+localStorage.getItem("link")+".jpg");
                     }
                    ...
                    $("#img_phuquoc img").show().animate({
                        'opacity': 1,
                    }, 2000);
                    $("#btnback").show().animate({
                        'top': '0px',
                       'margin': '1.5%',
                       'left': '0px'
                    }, 2000);
...
var Canvas = function Canvas(baseComponent, THREE) {
    ...
    return _Util2.default.extend(parent, {
        constructor: function init(player, options) {
            ...
            this.VRMode = false;
            //define scene
            this.scene = new THREE.Scene(); this.scene2 = new THREE.Scene();
            //define camera
            ...
            this.addMultiHotspot();
            
           var spriteMap = new THREE.TextureLoader().load( "img/logo.png" );
        ...
        enableVR: function enableVR() {
            ...
            this.cameraL = new THREE.PerspectiveCamera(50, this.width / this.height, 1, 2000);
            this.cameraR = new THREE.PerspectiveCamera(50, this.width / this.height, 1, 2000);
        },
